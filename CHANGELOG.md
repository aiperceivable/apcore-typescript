...
- **BREAKING: `additionalProperties` is honoured by the schema converter.** It was ignored, so an object declared `additionalProperties: false` accepted unknown keys, while apcore-python (pydantic `extra="forbid"`) and apcore-rust (jsonschema crate) both reject them. The object form (`additionalProperties: {type: "integer"}`) is honoured as well. **Impact:** apexe emits `"additionalProperties": false` on every generated input schema, so a call that passes any key not declared in the contract now fails with `SCHEMA_VALIDATION_ERROR` at the validation boundary, before the module runs — where it previously succeeded. This is the behaviour apcore-python and apcore-rust have always had; a caller relying on the old behaviour was relying on a JS-only defect. **Migration:** drop the undeclared keys, or declare them (`additionalProperties: true` / a sub-schema) in the contract.

- **BREAKING: a `type` array converts to a union, and its combinator siblings are enforced.** `{"type": ["string", "boolean"]}` — which apexe emits for value-optional flags — fell through to `Type.Unknown()`, so the property accepted a number, an object, anything. It now converts to a union of its members, with the type-specific option keywords kept per branch and the sibling assertion keywords (`enum`, `const`, `anyOf`, `oneOf`, `allOf`, `not`) intersected with the whole union, matching apcore-rust and apcore-python. The same intersection now also applies to a scalar `type`, so `{"type": "string", "enum": [...]}` enforces its `enum` (it did not before, on any released version). **Impact:** a value that was accepted only because the type or the enum was being dropped now fails with `SCHEMA_VALIDATION_ERROR`. **Migration:** none for a well-formed call; a call sending a value outside the declared type or enum was already rejected by the other two SDKs.

- **`format` neutralisation is scoped to one check instead of mutating the process-global registry.** TypeBox's `FormatRegistry` is process-global and shared with the host application, so the first implementation of the fix below was order-dependent in one direction and leaky in the other: a host that had registered a strict `email` checker before apcore loaded made apcore *reject* a malformed address (turning the SHOULD-level warning into `SCHEMA_VALIDATION_ERROR`), and every format apcore ever met stayed registered as accept-everything forever, so the host's own `Type.String({format: 'email'})` silently stopped rejecting garbage. `SchemaValidator` now overrides only the formats the schema at hand carries, for the duration of one synchronous TypeBox call, and restores the previous entries — including their absence — in a `finally`. Nothing can interleave inside that window, because `Value.Check` / `Value.Decode` / `Value.Errors` are synchronous. `jsonSchemaToTypeBox` no longer touches the registry at all. **Impact:** apcore no longer mutates the registry on the caller's behalf, so calling `Value.Check` **directly** on a converted schema that carries an unrecognised format now gets TypeBox's own reject-unknown-format behaviour instead of passing. Validate through `SchemaValidator` (which is what the executor does) and the annotation semantics hold regardless of what the host registered.

### Fixed

- **An unrecognised `format` no longer fails validation (apexe#32).** JSON Schema 2020-12 §7.2.1 puts `format` in the **format-annotation** vocabulary by default: assertion behaviour is opt-in, and a format value the implementation does not recognise is collected as an annotation and MUST NOT be treated as an assertion failure. TypeBox does the opposite — `Value.Check` rejects any string whose `format` is absent from its global `FormatRegistry` — so a contract carrying, for example, `format: "path"` made every call passing that field fail with `SCHEMA_VALIDATION_ERROR` before the module ran. Scanned apexe contracts were uncallable from a JS consumer (the path operands of `cli.find` / `cli.cp` / `cli.ls`) while the identical contract executed fine over apcore-rust. Every format apcore meets is now registered as an accept-everything checker at both boundaries — `jsonSchemaToTypeBox` and `SchemaValidator.validate` — and the format value is preserved on the schema. New module `schema/formats.ts` owns the recognised-format checkers and the registration helpers. The other half is unchanged by design and for the same reason: a **recognised** format whose value does not satisfy it still passes, with a `console.warn` at SHOULD level, because the default vocabulary makes `format` an annotation regardless of whether the implementation recognises it. apcore-python and apcore-rust behave the same way, and the conformance fixture `schema_hardening_formats.json` corroborates it (`valid: true`, `error_code: null`). Note that the fixture's `warn_logged: true` half is weaker evidence than it looks — in both apcore-rust and apcore-python the warning is emitted on a code path module invocation does not reach — so the specification, not the fixture, is the authority here.

...
- **Notice that relative path-typed values will one day resolve against one base ([apcore#113](https://github.com/aiperceivable/apcore/issues/113), spec §13.2).** `Config.load` now warns once per process when `projectRoot` differs from the CWD **and** the configuration carries at least one relative path-typed value — naming the affected keys, the project root and the CWD. Both conditions have to hold for the coming change to move anything; a blanket warning on every out-of-tree config would train everyone to ignore it, and apcore#113 rules it out explicitly. In the ordinary tier 2-5 project the first condition is false and nothing is printed. Relative values inherited from the `DEFAULTS` table count, because those re-root under a new base just as a written value does; `extensions.roots` counts when any element is relative, in either its bare-string or `{ root, namespace }` form, and is reported under the `extensions.roots[]` key §9.2.1 gives it.

  **No resolution behaviour changes in this release.** `SchemaLoader` still resolves `schema.root` against the CWD and `ACL.discover` still resolves `acl.root` against the config file's directory for every tier, both pinned by test so a premature adoption of the new base cannot land silently. Adopting `projectRoot` as *the* base for every path-typed key is a behaviour change to deployed configurations and therefore a major-version move; this is its deprecation phase. `_resetProjectRootDeprecationWarned()` is exported from `src/config.ts` as an internal test hook, on the same terms as `_resetMultiClassEnabledDeprecationWarned`.
